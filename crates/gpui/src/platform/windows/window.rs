...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        let is_child = params.parent_handle.is_some();

        let (mut dwexstyle, dwstyle) = if is_child {
            (WINDOW_EX_STYLE(0), WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS)
        ...
        if !disable_direct_composition && !is_child {
        ...
        let parent_hwnd = params.parent_handle.map(|h| HWND(h as *mut _));
        let (x, y, cx, cy) = if is_child {
            (0, 0, params.bounds.size.width.0 as i32, params.bounds.size.height.0 as i32)
        } else {
